...
class TriageOutput(BaseModel):
    agent_name: str = Field(
        description="The name of the specialized agent selected to handle the request. "
                    "Must be one of the names defined in the 'handoffs' list (e.g., 'Notification Manager') or use the product_agent tool for product queries."
    )
    reasoning: str = Field(
...
- **Notification Manager**: Use this handoff when you need to send welcome messages to new users. This agent will create and send personalized welcome messages.
...
- For welcome messages, use the 'Notification Manager' handoff
